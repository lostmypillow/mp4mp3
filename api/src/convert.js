...
export const handler = async (event) => {
    ...
    if (!bucketName || !objectKey) {
        console.log("Ignored: Missing bucket or object key in EventBridge payload.");
        return;
    }

    // 1. Check the file size
    if (objectSize > MAX_FILE_SIZE_BYTES) {
        console.warn(`File ${objectKey} rejected. Size (${objectSize} bytes) exceeds limit.`);
        ...
        try {
            ...
            console.error(`Failed to delete oversized file ${objectKey}:`, deleteErr);
        }
        return;
    }

    console.log(`File size is ${objectSize} bytes. Proceeding with conversion for ${objectKey}.`);
    ...
    try {
        ...
        try {
            ...
            console.error(`Failed to delete failed file ${objectKey}:`, deleteErr);
        }
        ...
        try {
            ...
            console.error(`Failed to delete processed file ${objectKey}:`, deleteErr);
        }
    }
};