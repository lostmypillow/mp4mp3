...
export const handler = async (event) => {
    ...
    try {
        console.warn(`Killswitch triggered by CloudWatch alarm. Disabling EventBridge rule: ${ruleName}`);

        const command = new DisableRuleCommand({ Name: ruleName });
        await client.send(command);

        console.log(`Successfully disabled rule: ${ruleName}. Conversion pipeline stopped.`);

        return {
            statusCode: 200,
            body: JSON.stringify({ message: `Rule ${ruleName} disabled successfully.` }),
        };
    } catch (error) {
